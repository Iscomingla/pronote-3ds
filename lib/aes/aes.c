/*
 * Tiny AES-128-CBC — public domain (unlicense)
 * Stripped from https://github.com/kokke/tiny-AES-c
 * Only CBC decrypt kept; no encrypt, no 192/256-bit support.
 */
...
static void InvMixColumns(state_t *state) {
    ...
    for (uint8_t i = 0; i < 4; i++) {
        ...
        (*state)[i][0] = (uint8_t)(xtime(xtime(xtime(a)^b^c^d)^b^d)^b^c^d);
        (*state)[i][1] = (uint8_t)(xtime(xtime(xtime(b)^c^d^a)^c^a)^c^d^a);
        (*state)[i][2] = (uint8_t)(xtime(xtime(xtime(c)^d^a^b)^d^b)^d^a^b);
        (*state)[i][3] = (uint8_t)(xtime(xtime(xtime(d)^a^b^c)^a^c)^a^b^c);
    }
}
