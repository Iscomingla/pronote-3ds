...
{
	quirc_float_t x0 = rect[0].x; quirc_float_t y0 = rect[0].y;
	quirc_float_t x1 = rect[1].x; quirc_float_t y1 = rect[1].y;
	quirc_float_t x2 = rect[2].x; quirc_float_t y2 = rect[2].y;
	quirc_float_t x3 = rect[3].x; quirc_float_t y3 = rect[3].y;
			    ...
			    quirc_float_t u, quirc_float_t v, struct quirc_point *ret)
{
	...
	ret->x = (int) rint((c[0]*u + c[1]*v + c[2]) * den);
	ret->y = (int) rint((c[3]*u + c[4]*v + c[5]) * den);
}
...
{
	quirc_float_t x = in->x, y = in->y;
	quirc_float_t den = (quirc_float_t)1 / (-c[0]*c[7]*y + c[1]*c[6]*y + (c[3]*c[7]-c[4]*c[6])*x + c[0]*c[4] - c[1]*c[3]);
...
static void flood_fill_line(struct quirc *q, int x, int y, int from, int to,
			    span_func_t func, void *user_data, int *leftp, int *rightp)
{
...
static struct quirc_flood_fill_vars *flood_fill_call_next(
			struct quirc *q, quirc_pixel_t *row,
			int from, int to, span_func_t func, void *user_data,
			struct quirc_flood_fill_vars *vars, int direction)
{
	int *leftp = (direction < 0) ? &vars->left_up : &vars->left_down;
	while (*leftp <= vars->right) {
		if (row[*leftp] == from) {
			struct quirc_flood_fill_vars *next_vars = vars + 1;
			int next_left;
			    ...
			    span_func_t func, void *user_data)
{
	...
	struct quirc_flood_fill_vars *next_vars = stack;
	int next_left;
	next_vars->y = y0;
	...
	while (true) {
		...
		if (vars > stack) { next_vars = vars - 1; continue; }
		break;
	}
}
...
static uint8_t otsu(const struct quirc *q)
{
	...
	while (length--) { uint8_t v = *ptr++; histogram[v]++; }
	quirc_float_t sum = 0;
	unsigned int i;
	for (i = 0; i <= 255; i++) sum += i * histogram[i];
	quirc_float_t sumB = 0; unsigned int q1 = 0;
	quirc_float_t max = 0; uint8_t threshold = 0;
	for (i = 0; i <= 255; i++) {
		q1 += histogram[i]; if (q1 == 0) continue;
		unsigned int q2 = numPixels - q1; if (q2 == 0) break;
		sumB += i * histogram[i];
		quirc_float_t m1 = sumB / q1, m2 = (sum - sumB) / q2;
		quirc_float_t m1m2 = m1 - m2;
		quirc_float_t variance = m1m2 * m1m2 * q1 * q2;
		if (variance >= max) { threshold = i; max = variance; }
	}
	return threshold;
}
...
static int region_code(struct quirc *q, int x, int y)
{
	...
	box->seed.x = x; box->seed.y = y; box->capstone = -1;
	flood_fill_seed(q, x, y, pixel, region, area_count, box);
	return region;
}

struct polygon_score_data { struct quirc_point ref; int scores[4]; struct quirc_point *corners; };
...
static void find_one_corner(void *user_data, int y, int left, int right)
{
	...
	int xs[2] = {left, right}, i;
	int dy = y - psd->ref.y;
	for (i = 0; i < 2; i++) {
		int dx = xs[i] - psd->ref.x, d = dx*dx + dy*dy;
...
static void find_other_corners(void *user_data, int y, int left, int right)
{
	...
	for (i = 0; i < 2; i++) {
		...
		int r  = xs[i]*-psd->ref.y + y*psd->ref.x;
		int scores[4] = {up, r, -up, -r};
		int j;
		for (j = 0; j < 4; j++) {
			if (scores[j] > psd->scores[j]) { psd->scores[j] = scores[j]; psd->corners[j].x = xs[i]; psd->corners[j].y = y; }
		}
	}
}
...
static void find_region_corners(struct quirc *q, int rcode, const struct quirc_point *ref, struct quirc_point *corners)
{
	...
	psd.scores[0] = i; psd.scores[2] = -i;
	i = region->seed.x * -psd.ref.y + region->seed.y * psd.ref.x;
	psd.scores[1] = i; psd.scores[3] = -i;
...
static void record_capstone(struct quirc *q, int ring, int stone)
{
	...
	capstone->qr_grid = -1; capstone->ring = ring; capstone->stone = stone;
	stone_reg->capstone = cs_index; ring_reg->capstone = cs_index;
...
{
	quirc_pixel_t *row = q->pixels + y * q->w;
	unsigned int x, run_length = 0, run_count = 0;
	int last_color = 0;
	unsigned int pb[5];
	...
	for (x = 0; x < q->w; x++) {
		...
		if (x && color != last_color) {
			...
			pb[4] = run_length; run_length = 0; run_count++;
			if (!color && run_count >= 5) {
				const int scale = 16;
				static const unsigned int check[5] = {1,1,3,1,1};
				unsigned int avg, err, i; int ok = 1;
...
static void find_alignment_pattern(struct quirc *q, int index)
{
	...
	struct quirc_point a, b, c;
	int size_estimate, step_size = 1, dir = 0;
	quirc_float_t u, v;
...
static quirc_float_t qlength(struct quirc_point a, struct quirc_point b)
{
	quirc_float_t x = abs(a.x-b.x)+1, y = abs(a.y-b.y)+1;
	return sqrt(x*x+y*y);
}
...
static void measure_grid_size(struct quirc *q, int index)
{
	...
	quirc_float_t ab = qlength(b->corners[0], a->corners[3]);
	quirc_float_t cs_ab = (qlength(b->corners[0],b->corners[3])+qlength(a->corners[0],a->corners[3]))/2.0;
	quirc_float_t ver_grid = (quirc_float_t)7.0*ab/cs_ab;
	quirc_float_t bc = qlength(b->corners[0], c->corners[1]);
	quirc_float_t cs_bc = (qlength(b->corners[0],b->corners[1])+qlength(c->corners[0],c->corners[1]))/2.0;
	quirc_float_t hor_grid = (quirc_float_t)7.0*bc/cs_bc;
	quirc_float_t gs = (ver_grid+hor_grid)*(quirc_float_t)0.5;
	int ver = (int)((gs-(quirc_float_t)(17.0-2.0))*(quirc_float_t)(1./4));
...
{ x+=3; y+=3; return fitness_cell(q,index,x,y)+fitness_ring(q,index,x,y,1)-fitness_ring(q,index,x,y,2)+fitness_ring(q,index,x,y,3); }
...
static int fitness_all(const struct quirc *q, int index)
{
	...
	for (i=1;i+1<ap_count;i++) { score+=fitness_apat(q,index,6,info->apat[i]); score+=fitness_apat(q,index,info->apat[i],6); }
	for (i=1;i<ap_count;i++) for (j=1;j<ap_count;j++) score+=fitness_apat(q,index,info->apat[i],info->apat[j]);
	return score;
}
...
{
	struct quirc_grid *qr = &q->grids[index];
	int best = fitness_all(q,index), pass, i;
	quirc_float_t adjustments[8];
	for (i=0;i<8;i++) adjustments[i] = qr->c[i]*(quirc_float_t)0.02;
	for (pass=0;pass<5;pass++) {
		for (i=0;i<16;i++) {
			int j=i>>1, test;
			quirc_float_t old=qr->c[j], step=adjustments[j], nw;
			nw = (i&1) ? old+step : old-step;
			qr->c[j] = nw; test = fitness_all(q,index);
...
static void rotate_capstone(struct quirc_capstone *cap, const struct quirc_point *h0, const struct quirc_point *hd)
{
	...
	int j, best=0, best_score=INT_MAX;
	for (j=0;j<4;j++) {
...
static void record_qr_grid(struct quirc *q, int a, int b, int c)
{
	...
	if ((q->capstones[b].center.x-h0.x)*-hd.y+(q->capstones[b].center.y-h0.y)*hd.x>0) {
		int swap=a; a=c; c=swap; hd.x=-hd.x; hd.y=-hd.y;
	}
	qr_index=q->num_grids; qr=&q->grids[q->num_grids++];
	memset(qr,0,sizeof(*qr));
	qr->caps[0]=a; qr->caps[1]=b; qr->caps[2]=c; qr->align_region=-1;
	for (i=0;i<3;i++) { struct quirc_capstone *cap=&q->capstones[qr->caps[i]]; rotate_capstone(cap,&h0,&hd); cap->qr_grid=qr_index; }
	measure_grid_size(q,qr_index);
	if (!line_intersect(&q->capstones[a].corners[0],&q->capstones[a].corners[1],
			    &q->capstones[c].corners[0],&q->capstones[c].corners[3],&qr->align)) goto fail;
	if (qr->grid_size>21) {
...
struct neighbour { int index; quirc_float_t distance; };
struct neighbour_list { struct neighbour n[QUIRC_MAX_CAPSTONES]; int count; };
...
static void test_neighbours(struct quirc *q, int i, const struct neighbour_list *hlist, const struct neighbour_list *vlist)
{
	for (int j=0;j<hlist->count;j++) {
		...
		for (int k=0;k<vlist->count;k++) {
			...
			quirc_float_t sq=fabs((quirc_float_t)1.0-hn->distance/vn->distance);
			if (sq<(quirc_float_t)0.2) record_qr_grid(q,hn->index,i,vn->index);
		}
	}
}
...
static void test_grouping(struct quirc *q, unsigned int i)
{
	...
	hlist.count=0; vlist.count=0;
	for (j=0;j<q->num_capstones;j++) {
		struct quirc_capstone *c2=&q->capstones[j];
		quirc_float_t u, v;
		if (i==j) continue;
		perspective_unmap(c1->c,&c2->center,&u,&v);
		u=fabs(u-(quirc_float_t)3.5); v=fabs(v-(quirc_float_t)3.5);
		if (u<(quirc_float_t)0.2*v) { struct neighbour *n=&hlist.n[hlist.count++]; n->index=j; n->distance=v; }
		if (v<(quirc_float_t)0.2*u) { struct neighbour *n=&vlist.n[vlist.count++]; n->index=j; n->distance=u; }
	}
...
static void pixels_setup(struct quirc *q, uint8_t threshold)
{
	...
	uint8_t *source=q->image; quirc_pixel_t *dest=q->pixels;
	int length=q->w*q->h;
	while (length--) { uint8_t v=*source++; *dest++=(v<threshold)?QUIRC_PIXEL_BLACK:QUIRC_PIXEL_WHITE; }
}
