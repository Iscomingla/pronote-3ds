...
// citro2d system-font text buffer (one per frame is fine for our use)
...
// ---------------------------------------------------------------------------
void ui_init(void) {
    // citro3d must be init before citro2d
    ...
    // Small text buffer — enough for all strings in a single frame
...
void ui_text(float x, float y, float size, u32 colour, const char *str) {
    C2D_Text t;
    C2D_TextFontParse(&t, C2D_FontLoadSystem(), s_tbuf, str);
    C2D_TextOptimize(&t);
